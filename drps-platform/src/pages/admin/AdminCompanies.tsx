...
const AdminCompanies: React.FC = () => {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .eq('empresa_id', showColabModal.id)
      .eq('role', 'colaborador')
      .then(({ data }) => {
  ...
  const handleDelete = async () => {
    setIsSubmitting(true);
    setDeleteError(null);
    const { error } = await supabase.from('empresas').delete().eq('id', showDeleteModal.id);
    if (error) {
      const isFK = error.code === '23503' || error.message?.toLowerCase().includes('foreign key');
      setDeleteError(
        isFK
          ? 'Existem dados vinculados a esta empresa. Aplique a migration de CASCADE no banco de dados e tente novamente.'
  ...
  const handleAddColab = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result?.error || 'Erro ao criar colaborador.');
      setNewColab({ email: '', full_name: '', password: '' });
      // Re-fetch collaborators list
      const { data } = await supabase.from('profiles').select('id, full_name, email, role').eq('empresa_id', showColabModal.id).eq('role', 'colaborador');
      setColabProfiles(data || []);
    } catch (err: any) {
      alert(err.message);
    }
