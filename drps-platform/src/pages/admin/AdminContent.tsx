...
const AdminContent: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (newLecture.tipo_video === 'upload' && videoFile) {
      ...
      const { error: uploadError } = await supabase.storage.from('palestras').upload(`${newLecture.empresa_id}/${fileName}`, videoFile);
      if (!uploadError) {
        const { data: urlData } = supabase.storage.from('palestras').getPublicUrl(`${newLecture.empresa_id}/${fileName}`);
  ...
  const handleDelete = async () => {
    setIsSubmitting(true);
    const { error } = await supabase.from('palestras').delete().eq('id', showDeleteModal.id);
    if (error) alert(error.message);
    else { setShowDeleteModal(null); refreshData(); }
    setIsSubmitting(false);
  };
  ...
  const handleEditSave = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsSubmitting(true);
    const { error } = await supabase.from('palestras').update({
      titulo: editLecture.titulo,
      descricao: editLecture.descricao,
      url_video: editLecture.url_video,
    }).eq('id', showEditModal.id);
    if (error) alert(error.message);
    else { setShowEditModal(null); refreshData(); }
    setIsSubmitting(false);
  };
          ...
          {allLectures.map(lect => (
            <div key={lect.id} className="bg-white rounded-[32px] border border-slate-100 overflow-hidden shadow-sm hover:shadow-2xl transition-all group relative border-b-4 border-b-brand-accent">
              <div className="aspect-video bg-slate-900 flex items-center justify-center relative overflow-hidden">
                ...
                <div className="absolute top-4 left-4 bg-white/10 backdrop-blur-md text-white text-[10px] px-3 py-1 rounded-full font-bold uppercase tracking-widest">{lect.empresas?.nome}</div>
