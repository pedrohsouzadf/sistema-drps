...
const AdminSchedule: React.FC = () => {
  ...
  const handleConfirm = async (apptId: string, data_hora: string, link_reuniao: string) => {
  ...
  const handleUpdateLink = async (apptId: string) => {
    const { error } = await supabase.from('agendamentos').update({ link_reuniao: tempLink }).eq('id', apptId);
    if (!error) { setEditingLink(null); refreshData(); }
  };
