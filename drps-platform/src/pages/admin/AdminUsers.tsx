...
const AdminUsers: React.FC = () => {
  ...
  const fetchProfiles = async () => {
    ...
    const [profilesResult, authResponse] = await Promise.all([
      supabase.from('profiles').select('*, empresas(nome)').order('full_name', { ascending: true }),
      fetch('/api/list-users').catch(() => null),
    ]);
    ...
    if (authResponse) {
      const authResult = await safeJson(authResponse);
      if (authResult?.missingKey) {
        missingKey = true;
      } else if (Array.isArray(authResult?.users)) {
        emailMap = new Map(authResult.users.map((u: any) => [u.id, u.email]));
      }
    } else {
      missingKey = true;
    }
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result?.error ||
          `Erro ${response.status}. Em desenvolvimento local, use o painel do Supabase → Authentication → Users.`
        );
      }
      setShowModal(false);
      ...
      <div className="bg-amber-50 border border-amber-100 p-5 rounded-[24px] flex gap-4">
        ...
        <div>
          ...
          <p className="text-amber-700/80 text-xs leading-relaxed">
            A criação e exclusão requerem a variável{' '}
            <code className="bg-white px-1 rounded">SUPABASE_SERVICE_ROLE_KEY</code> configurada na
            Vercel. Em desenvolvimento local, os colaboradores podem se auto-cadastrar em{' '}
            <strong>/login?role=colaborador</strong>, ou crie diretamente pelo painel do Supabase →
                ...
                <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-3">
                      ...
                      <p className="font-bold text-slate-800">{p.full_name || 'Sem Nome'}</p>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-5">
                    <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                      p.role === 'psicologo' ? 'bg-indigo-50 text-indigo-600' : 'bg-brand-light text-brand-primary'
                    }`}>
                  ...
                  <td className="px-6 py-5 text-sm">
                    {p.role === 'psicologo' ? (
                      ...
                      <div className="flex items-center gap-2 text-slate-600 font-medium">
                        ...
                        {p.empresas?.nome || <span className="text-slate-300 italic text-xs">Não vinculada</span>}
                      </div>
                    )}
                  </td>
