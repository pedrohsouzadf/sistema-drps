...
import { 
  LayoutDashboard, Building2, Calendar as CalendarIcon, 
  PlayCircle, LogOut, Activity, Menu, X, Users
} from 'lucide-react';
...
const AdminLayout: React.FC = () => {
  ...
  const menuItems = [
    ...
    { icon: <Building2 size={20}/>, label: "Clientes", path: "/admin/clientes" },
    { icon: <Users size={20}/>, label: "Usuários", path: "/admin/usuarios" },
    { icon: <CalendarIcon size={20}/>, label: "Agenda", path: "/admin/agenda" },
    { icon: <PlayCircle size={20}/>, label: "Palestras", path: "/admin/palestras" },
  ];
      ...
      `}>
        <div className="p-6 border-b border-white/5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="text-orange-500 font-bold">DRPS <span className="text-[10px] bg-brand-accent text-white px-2 py-0.5 rounded-full ml-1">ADMIN</span></div>
          </div>
        ...
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-8 sticky top-0 z-30">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:block">
              <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-1">Área do Psicólogo</h2>
          ...
          <div className="flex items-center gap-6">
             <div className="text-right hidden md:block">
               <p className="text-sm font-bold text-slate-800">{profile?.nome || profile?.full_name}</p>
               <p className="text-xs text-brand-medium font-medium">Administrador</p>
             </div>
             <div className="w-12 h-12 bg-slate-100 border border-slate-200 rounded-2xl flex items-center justify-center text-brand-primary font-bold shadow-inner relative group cursor-pointer" onClick={handleLogout}>
               {(profile?.nome || profile?.full_name)?.[0]}
