...
const AdminCompanyReport: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const [empresa, resps] = await Promise.all([
        apiGet<any>(`/empresas/${empresaId}`),
        apiGet<any[]>(`/respostas?empresa_id=${empresaId}`),
      ]);
      if (empresa) { setCompany(empresa); }
  ...
  useEffect(() => {
    if (!empresaId) return;
    apiGet<any[]>(`/prob-topico?empresa_id=${empresaId}`).then(data => {
  ...
  const handleProbChange = async (topico: string, valor: number) => {
    ...
    await apiPut('/prob-topico', [{ empresa_id: empresaId, setor: probSector, topico, valor }]);
    setProbSaving(null);
  };
