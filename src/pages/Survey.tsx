...
const Survey: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      if (slug) {
        const data = await apiGet<any>(`/empresas?slug=${slug}`);
        if (data) { setSelectedCompany(data.id); setStep('info'); }
  ...
  const handleSubmit = async () => {
    ...
    try {
      await apiPost('/respostas', { empresa_id: selectedCompany, cargo, setor, respostas: responseArray });
      setStep('success');
    } catch (e: any) {
      alert('Erro ao processar respostas: ' + e.message);
    } finally {
      setLoading(false);
    }
  };
