...
const ColaboradorSchedule: React.FC = () => {
  ...
  const loadSlots = useCallback(async () => {
    ...
    try {
      ...
      occupied = occupied.filter(a => a.status !== 'cancelado' && a.data_hora >= startDate.toISOString() && a.data_hora <= endDate.toISOString());
    } catch {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (e.message.includes('409') || e.message.includes('reservado')) {
        ...
        setSubmitError('Erro ao enviar solicitação: ' + e.message);
      }
    }
